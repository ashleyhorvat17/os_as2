//a1705630 Eamon Fitzgerald
//a1770422 Tom Zhu
// a1765128 Ashley Horvat
// OS Team 1
/*
...
int main(int argc, char *argv[])
{
    ...
    std::deque<int> aQueue; // Arrival queue
    std::deque<int>::iterator qit;
    std::deque<int>::iterator aqit;
    ...
    for (int current_time = 0; !all_done; current_time++) {
        ...
        while (!arrival_events.empty() && (current_time == arrival_events[0].event_time)) {
            // int p = customers.at(arrival_events[0].customer_id).priority;
            // int l = customers.at(arrival_events[0].customer_id).slots_remaining;


            aQueue.push_back(arrival_events.at(0).customer_id);
            arrival_events.pop_front();
            aCount++;
            // int pity = 0;
            // if (queue.empty() && aQueue.empty()) {
            //     queue.push_back(arrival_events[0].customer_id);
            // } else {
                // qit = queue.begin();
                // for (int i=0;i<queue.size();i++) {
                //     // if (p < customers.at(queue[i]).priority) {
                //     //     pity++;
                //     // }
                //     // if (pity >= 3) {
                //     //     pity = 0;
                //     //     queue.insert(qit,arrival_events[0].customer_id); 
                //     //     break;
                //     // }
                //     if (p >= customers.at(queue[i]).priority && l < customers.at(queue[i]).slots_remaining) {
                //         queue.insert(qit,arrival_events[0].customer_id); 
                //         break;
                //     }
                //     if (i >= queue.size()-1) {
        ...
        if (current_id >= 0)
        {
            if (current_time == time_out)
            {
                ...
                if (customers[current_id].slots_remaining > 0)
                {
                    ...
                    int p = customers.at(current_id).priority;
                    int l = customers.at(current_id).slots_remaining;
                    if (queue.empty()) {
                        queue.push_back(current_id);
                    } else {
                        qit = queue.begin();
                        for (int i=0;i<queue.size();i++) {
                            // if (l < customers.at(queue[i]).slots_remaining) {
                            //     queue.insert(qit,current_id); 
                            //     break;
                            // }
                            if (p <= customers.at(queue[i]).priority && l < customers.at(queue[i]).slots_remaining) {
                                queue.insert(qit,current_id); 
        ...
        {
            if (!aQueue.empty()) {
                current_id = aQueue.front();
                aQueue.pop_front();
                if (1 > customers[current_id].slots_remaining) {
                    time_out = current_time + customers[current_id].slots_remaining;
                } else {
                    time_out = current_time + 1;
                }
